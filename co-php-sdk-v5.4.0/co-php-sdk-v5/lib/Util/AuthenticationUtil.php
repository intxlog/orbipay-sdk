...
class AuthenticationUtil
{
    ...
    public static function computeHMACSHA256Hash($url, $method, $queryParams,
       ...
       echo "<script>console.log('in computeHMACSHA256Hash ');</script>";
        ...
        $authenticationString .= trim((strtoupper($method) . APIConstants::SEPARATOR));

        $authenticationString .= trim(($subPath . APIConstants::SEPARATOR));
    ...
    private static function getQueryString($queryParams){
        ...
        return $queryString . APIConstants::SEPARATOR;
    }

    private static function getHeaderString($headerParams){
        $headerString = '';
        $headers = array("client_id", "digi_sign");
        //CHECKOUT_HEADERS = ['client_id','digi_sign'];
        ksort($headerParams);
        $filteredHeaders = [];
        ...
        return $headerString . APIConstants::SEPARATOR;
    }
